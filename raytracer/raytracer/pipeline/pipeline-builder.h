...
namespace raytracer
{
    namespace pipeline
    {
        namespace _private_
        {
            ...
            {
                PipelineBuilder(T1& value, std::shared_ptr<Consumer<T1>> first, std::shared_ptr<Producer<T2>> last)
                    : value(value), first(first), last(last) { }

                T1& value;
                std::shared_ptr<Consumer<T1>> first;
                std::shared_ptr<Producer<T2>> last;
            };
            ...
            template<typename T1, typename T2>
            PipelineBuilder<T1, T2> operator >> (const PipelineBuilderStart<T1> start, std::shared_ptr<Processor<T1, T2>> processor)
            {
                return PipelineBuilder<T1, T2>(start.value, processor, processor);
            }

            template<typename T1, typename T2, typename C>
            std::enable_if_t<!is_processor<C>::value> operator >> (const PipelineBuilder<T1, T2>& builder, std::shared_ptr<C> consumer)
            {
            ...
            PipelineBuilder<T1, std::enable_if_t<is_processor<P>::value, typename P::output_type>> operator >> (const PipelineBuilder<T1, T2>& builder, std::shared_ptr<P> processor)
            {
